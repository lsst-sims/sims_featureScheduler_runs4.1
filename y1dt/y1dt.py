...
from rubin_scheduler.site_models import ScheduledDowntimeData, UnscheduledDowntimeMoreY1Data, UnscheduledDowntimeData
...
class UnchedDtVar(UnscheduledDowntimeMoreY1Data):
    def __init__(self, start_time, downtime_scale=0.5, **kwargs):
        self.downtime_scale = downtime_scale
    ...
    def make_data(self, end_of_start=380):
        ...
        for night, (sunset, sunrise) in enumerate(zip(self.sunsets, self.sunrises)):
            ...
            if night < end_of_start:
                ...
                # should be .. 5%?
                nightly_threshold = self.downtime_scale * (1 - night / (end_of_start + 45))
...
def downtimes(mjd_start=SURVEY_START_MJD, downtime_scale=0.5):
    sched_downtime_data = ScheduledDowntimeData(Time(mjd_start, format="mjd"))
    unsched_downtime_data = UnchedDtVar(Time(mjd_start, format="mjd"), downtime_scale=downtime_scale)
...
def run_sched(
    ...
    dts = downtimes(downtime_scale=downtime_scale)

    observatory = ModelObservatory(nside=nside, mjd_start=mjd_start, sim_to_o=sim_to_o, downtimes=dts)
...
def gen_scheduler(args):
    ...
    too = ~args.no_too
    downtime_scale = args.downtime_scale
    ...
    fileroot, extra_info = set_run_info(dbroot=dbroot, file_end="dts%.1f_v4.1_" % downtime_scale, out_dir=out_dir)
...
def sched_argparser():
    ...
    parser.add_argument("--downtime_scale", type=float, default=0.5)
